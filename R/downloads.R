...
<<<<<<< HEAD
=======
>>>>>>> 51cfa39a16787e960a31690a454044c18877e9d7
#KW
...
<<<<<<< HEAD
#KW
.davies.2014 <- function(...){
    data <- read.csv(ft_get_si("10.6084/m9.figshare.3560811.v1", "zooplankton_abundance_data.csv"))
    names(data) <- tolower(names(data))
    data$genus_species <- tolower(paste(data$genus, data$species, sep="_"))
    metadata <- data[,c("project_id", "taxon_name", "functional_group", "family", "genus", "species", "latitude", "longitude", "sample_year", "sample_month", "sample_day", "sample_time_utc")]
    data <- data[,!names(data) %in% names(metadata)]
    units <- c(NA, "abundance m^-3", "abundance m^-2", "m", "m", "mm", NA)
    return(.df.melt(data, "genus_species", units, metadata))
}

#KW
.toth.2014 <- function(...){
    data <- read.csv(ft_get_si("10.6084/m9.figshare.3559569.v1", "KMD13_traits.csv"))
    names(data) <- tolower(names(data))
    data$genus_species <- tolower(gsub(" ", "_", data$dwc.scientificname))
    metadata <- data[,c("dwc.scientificname", "dwc.vernacularname", "dwc.order", "dwc.family", "code")]
    data <- data[,!names(data) %in% names(metadata)]
    levels(data$diet_1) <- c("carnivore", "omnivore", "herbivore")
    units <- c(NA, "kg", NA, NA, NA, NA, NA, NA, NA)
    return(.df.melt(data, "genus_species", units, metadata))
}
